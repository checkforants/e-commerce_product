...
import cl from './Navigator.module.scss'
...
const Navigator = (props:any) => {
	...
	const [clickedPage, setClickedPage] = useState('collections')
	...
	return (
		<div className=''>
			<div className='navigator fixed w-4/5 h-[70px] z-10 md:h-[90px] md:px-3 mx-auto pt-[4px] md:pt-2 flex flex-row justify-between border-b-2 box-border  bg-white'>
				<div className='flex flex-row-reverse md:flex-row  md:space-x-7 mt-3 md:mt-5'>
					<Link className='z-20 ml-[30px] md:ml-0' to="/">
					...
					<div className='titles flex flex-row space-x-5 md:space-x-8'>
						<Link onClick={()=>setClickedPage('collections')} className={clickedPage=='collections'?'active menu_item pb-8 box-border ':'menu_item pb-8 box-border'} to="/">Collections</Link>
						<Link onClick={()=>setClickedPage('men')} className={clickedPage=='men'?'active menu_item pb-8 box-border ':'menu_item pb-8 box-border'} to="/men">Men</Link>
						<Link onClick={()=>setClickedPage('women')} className={clickedPage=='women'?'active menu_item pb-8 box-border ':'menu_item pb-8 box-border'} to="/women">Women</Link>
						<Link onClick={()=>setClickedPage('about')} className={clickedPage=='about'?'active menu_item pb-8 box-border ':'menu_item pb-8 box-border'} to="/about">About</Link>
						{props.user.email?<Link onClick={()=>setClickedPage('createItem')} className={clickedPage=='createItem'?'active menu_item pb-8 box-border ':'menu_item pb-8 box-border'}to="/newItem">CreateItem</Link>:''}
					</div>
				</div>
				<div className='flex flex-row space-x-4 mt-4 z-30 md:mt-6 '>
					{/* <Plus></Plus> */}
					<div className='relative pl-2 lg:pl-10' onMouseEnter={()=>setCart(true)} onMouseLeave={()=>setCart(false)}>
						...
						<div className='w-[19px] h-[12px] absolute top-[-5px] right-[-5px] text-[8px] text-white font-bold bg-orange-400 text-center rounded-[40%]'>0</div>
						{cart?<Cart/>:''}
					</div>
			...
			<input  checked={menuModal} onChange={(e)=>setMenuModal(e.target.checked)} className={`w-[30px] fixed  z-40  ${cl.checkbox} md:hidden`} type="checkbox" name="" id="chb" />
					<div className={cl.hamburgerLines}>
						...
						<span className={`${cl.line} ${cl.line3}`}></span>
					</div>
		</div>
	);
};
